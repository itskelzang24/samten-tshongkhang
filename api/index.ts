...
function toProduct(r: any) { return { ID: r.id, Name: r.name, Category: r.category, Unit: r.unit, Cost: num(r.cost), Selling: num(r.selling), Stock: num(r.stock), MinStock: num(r.min_stock), Vendor: r.vendor, UpdatedAt: r.updated_at }; }
function toBill(r: any) { return { BillNo: r.bill_no, DateTime: r.date_time, CustomerName: r.customer_name, CustomerContact: r.customer_contact, Method: r.method, TransferId: r.transfer_id, User: r.user, Subtotal: num(r.subtotal), GSTTotal: num(r.gst_total), GrandTotal: num(r.grand_total), Status: r.status, UpdatedAt: r.updated_at }; }
function toLine(r: any) { return { BillNo: r.bill_no, LineId: r.line_id, DateTime: r.date_time, ItemID: r.item_id, ItemName: r.item_name, Qty: num(r.qty), Rate: num(r.rate), UnitCost: num(r.unit_cost), LineType: r.line_type, GST_Rate: num(r.gst_rate), GST_Amount: num(r.gst_amount), LineTotal: num(r.line_total), User: r.user, Status: r.status, UpdatedAt: r.updated_at }; }
...
async function reverseBillLines(billNo: string, user: string) {
  ...
  for (const ln of lines) {
    const qty = num(ln.qty); const lineType = String(ln.line_type || '').toUpperCase();
...
async function updateProductStock(itemId: string, qtyChange: number, user: string, refType: string, refNo: string, refLineId: string, gstRate: number, gstAmount: number) {
  ...
  const p = rows[0]; const newStock = num(p.stock) + num(qtyChange);
...
const getHandlers: Record<string, Handler> = {
  ...
  listProducts: async (p) => {
    const q = String(p.q || ''); const limit = parseInt(String(p.limit || '100'), 10); const start = parseInt(String(p.start || '0'), 10);
    let rows;
  ...
  getProduct: async (p) => {
    const id = String(p.productId || ''); if (!id) return null;
  ...
  listBills: async (p) => {
    const limit = parseInt(String(p.limit || '100'), 10); const offset = parseInt(String(p.offset || '0'), 10);
    const start = p.start ? String(p.start) : null; const end = p.end ? String(p.end) : null;
  ...
  getBill: async (p) => {
    const billNo = String(p.billNo || ''); if (!billNo) return null;
    ...
    const bill = toBill(bRows[0]); (bill as any).lines = lRows.map(toLine);
    return bill;
  },
  getDashboardData: async () => {
    ...
    return { lowStock, chartData, monthlySales, paymentMethods, totalSales: num(billSummary[0]?.total_sales), totalTransactions: num(billSummary[0]?.total_transactions), totalItemsSold: num(itemsSoldRow[0]?.total) };
  },
  getProfitData: async (p) => {
    ...
    const [products, purchases, billRows, lineRows] = await Promise.all([
      sql`SELECT id, cost FROM products`, sql`SELECT item_id, cost FROM purchase_transactions`,
...
const postHandlers: Record<string, Handler> = {
  ...
  saveBill: async (body) => {
    const billNo = body.billNo || body.BillNo; const customerName = body.customerName || body.CustomerName || '';
    const method = body.method || body.Method || ''; const user = body.user || body.User || '';
  ...
  saveProduct: async (body) => {
    const p = body.product || {}; let id = p.ID || p.id;
    if (!id) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const h = getHandlers[action];
      if (!h) return res.status(400).json({ error: 'Invalid action', action });
      return res.json(await h(req.query as any));
    }
    ...
    if (req.method === 'POST') {
      ...
      const h = postHandlers[action];
      if (!h) return res.status(400).json({ error: 'Invalid action', action });
      return res.json(await h(body));
    }
